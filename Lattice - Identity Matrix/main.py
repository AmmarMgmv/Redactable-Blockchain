...
def main():
    print("Initializing Quantum-Resistant Lattice-based Blockchain")
    ...
    test_valid_block_modification(my_blockchain, block_index=1, 
                                 new_data="Updated Transaction: Parker -> Tom, $75")
...
def test_valid_block_modification(blockchain, block_index, new_data):
    ...
    # Use the correct trapdoor (vector_s) for authorized modification
    success = block.modify_block(new_data, block.vector_s)
...
def test_invalid_block_modification(blockchain, block_index, new_data):
    ...
    # Create a fake trapdoor (random vector of the same shape)
    n = block.matrix_A.shape[0]
    fake_trapdoor = np.random.randint(0, block.q, size=(n, 1), dtype=np.int64)
